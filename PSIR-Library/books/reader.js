...
const activateSentenceNotes=article=>{
 ...
 const notesScript=document.createElement('script');
  notesScript.src='../../shared/sentence-notes.js?v=20260831-hidden-prose';
...
let readingLayerLoading=false;
const activateReadingLayer=(article,book)=>{
 const applyLayer=()=>window.PSIRReadingLayer?.apply(article,{bookId:id,sections:book.sections});
 if(window.PSIRReadingLayer)return applyLayer();
 if(readingLayerLoading)return;
 readingLayerLoading=true;
 const layerScript=document.createElement('script');
 layerScript.src='psir-reading-layer.js?v=20260831-2';
 layerScript.addEventListener('load',applyLayer);
 layerScript.addEventListener('error',()=>{readingLayerLoading=false;});
 document.head.append(layerScript);
};
...
const renderBook=()=>{
 ...
 if(book){
  ...
  document.querySelector('#book-number').textContent=`Book ${id} of 20`;
  ...
  activateReadingLayer(article,book);
