...
const activateBeginnerVisuals=article=>{
 const applyVisuals=()=>window.BeginnerLibraryVisuals?.apply(article,{subject:'history',sections:book.sections});
...
if(book){
 document.title=`${book.title} · Itihasa`;
 const toc=document.querySelector('#toc'),article=document.querySelector('#article');
 document.querySelector('#book-number').textContent=`Book ${id} of ${book.total||13}`;
...
addEventListener('scroll',()=>{const root=document.documentElement;const distance=root.scrollHeight-root.clientHeight;document.querySelector('.reading-progress').style.width=`${distance?100*root.scrollTop/distance:0}%`;localStorage.setItem(`hist-progress-${id}`,root.scrollTop)});
addEventListener('load',()=>{const position=Number(localStorage.getItem(`hist-progress-${id}`)||0);if(position>300&&confirm('Continue where you left off?'))scrollTo(0,position)});
